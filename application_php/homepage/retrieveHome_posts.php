...
$post_amount = $_GET['more'];

$post_amount = $post_amount*15;


include "../dbinclude.php";
 
$sql = "SELECT status_post.Status_id, status_post.Status_text, status_post.Status_type, status_post.Status_time, status_post.User_id, status_post.hashtag, status_post.Youtube_embed, images_files.Image_Name, music.Music_File_Name, music.Music_track, music.Music_album, music.Music_artist, music.Music_cover, login_user.Login_name, user_details.User_specialty, user_details.User_id,  (
...
SELECT COUNT(comments_posts.Status_id)
FROM comments_posts
WHERE comments_posts.Status_id = status_post.Status_id
) AS comment_amount
...
$res = mysqli_query($con,$sql);
 ...
 $response = $con->query($sql);
	...
	$amount = mysqli_num_rows($response);
...
while ($row = $response->fetch_assoc()) {
	...
	$posted_comment_amount = $row['comment_amount'];
