...
if(file_exists($target_dir.$saveName)) unlink($target_dir.$saveName);
    ...
    //$Ialbum = mysql_real_escape_string($con,$_POST['Ialbum']);
			...
			if ( mysqli_query($con,"INSERT INTO status_post(Status_text,Status_type,Status_time,User_id) VALUES('Changed Cover picture...','picture status','$status_post_time','$user_id')")) {
 ...
 $statusid= null;
	...
	mysqli_query($con,"Update images_files set Image_Name = '$saveName' , Image_date = '$image_time', Status_id = '$statusid' where User_id ='$user_id' and Cover_image = 'true'");
